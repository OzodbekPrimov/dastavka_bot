...
def inline_handler(update, context):
    ...
    if data_sp[0] == "category":
        ...
        if len(data_sp) == 2 and data_sp[1] == "clear":
            ...
            buttons = [
                [
                    InlineKeyboardButton(text=f"{globals.BUYURTMA_BERISH[db_user['lang_id']]}",  callback_data="order"),
                    InlineKeyboardButton(text=f"{globals.TOZALASH[db_user['lang_id']]}",  callback_data="cart_clear")
                ],
        ...
        if len(data_sp) > 1 and data_sp[1] == "payment":
            ...
            query.message.delete()
            query.message.reply_text(
                text=globals.SEND_LOCATION_TEXT[db_user["lang_id"]],
            ...
            query.message.edit_reply_markup(
                reply_markup=InlineKeyboardMarkup(
                    [[
                        InlineKeyboardButton(text=f"{globals.TULOV[db_user['lang_id']]}", callback_data="order_payment_1"),
                        InlineKeyboardButton(text=f"{globals.KARTA[db_user['lang_id']]}", callback_data="order_payment_2")
                    ]]
                )
            )
