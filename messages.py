...
def message_handler(update, context):
    ...
    if state == 0:
        ...
        if not lang_id:
            ...
            buttons = [
                [KeyboardButton(text=globals.BTN_SEND_CONTACT[lang_id], request_contact=True)]
            ]
            check(update, context)
...
################## lesson-4 ###################
        ...
        if message == globals.BTN_ORDER[db_user['lang_id']]:
            ...
            if context.user_data.get("carts", {}):
                ...
                text = f"{globals.AT_KORZINKA[lang_id]}:\n\n"
                ...
                buttons.append(
                    [InlineKeyboardButton(text=globals.BTN_KORZINKA[lang_id], callback_data="cart")])
        ...
        elif message == globals.BTN_MY_ORDERS[lang_id]:
            if context.user_data.get("carts", {}):
                carts = context.user_data.get("carts")
                text = "\n"
                ...
                update.message.reply_text(
                    text=f"<b>{globals.INFORMATION[db_user['lang_id']]}:</b>\n\n"
                         f"👤 <b>{globals.FIST_LAST_TEXT[db_user['lang_id']]}:</b> {db_user['first_name']} {db_user['last_name']}\n"
                         f"📞 <b>{globals.NUMBER_TEXT[db_user['lang_id']]}:</b> {db_user['phone_number']} \n\n"
                         f"📥 <b>{globals.ORDER_NAME[db_user['lang_id']]}:</b> \n"
                         f"{text}",
                    parse_mode='HTML'
                )
        ...
        elif message == globals.BTN_ABOUT_US[lang_id]:
            update.message.reply_text(
                text=globals.ABOUT_COMPANY[db_user['lang_id']],
                parse_mode="HTML"
            )

        elif message == globals.BTN_SETTINGS[lang_id]:
            buttons = [
